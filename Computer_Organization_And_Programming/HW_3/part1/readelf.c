  ...
  for (i = 0, section = filedata->section_headers;
	...
	{		
		if (section->sh_type != SHT_NULL)
		{
			...
			char* lee_name = (char*)malloc(strlen(name)+7);
			const char* lee = "_<3LEE\0";
			strcpy (lee_name, name);
			strcpy (lee_name+strlen(name), lee);
			print_symbol (-17, lee_name);
			free(lee_name);
		}
		else print_symbol (-17, SECTION_NAME_PRINT (section));
...
process_symbol_table (Filedata * filedata)
{
  ...
  if ((filedata->dynamic_info[DT_HASH] || filedata->dynamic_info_DT_GNU_HASH)
	...
	for (si = 0; si < num_syms; si++)
	{
		if (ELF_ST_BIND((si + symtab)->st_info) == STB_GLOBAL)
		{
			 global_counter ++;
		}
	}
