...
ASElement productCopy(ASElement product)
{
    ...
    product_copy->product_fixed_price = current_product->product_fixed_price;
    product_copy->product_base_price = current_product->product_base_price;
    ...
    case MATAMIKYA_INTEGER_AMOUNT:
        {
            ...
            if(diff <= MINIMAL_ERROR){ // check if the distance between rounded number and original valid.
                result = true;
            }
            break;
        }
...
static Product getBestSeller(Matamikya matamikya)
{
    ...
    double best_income = 0;
    AS_FOREACH(Product, iterator, matamikya->warehouse_products)
    {
        temp_income = iterator->product_income;
        best_income = best_seller->product_income;
        if ((temp_income) > (best_income)) {
...
Matamikya matamikyaCreate()
{
    ...
    new_warehouse->warehouse_products = asCreate(productCopy, productDestroy, productCompare); 
                              ...
                              MtmFreeData freeData, MtmGetProductPrice prodPrice)
{
    ...
    new_product->product_fixed_price = prodPrice(customData, amount);
    new_product->product_base_price = prodPrice(customData, 1);
    new_product->product_prodPrice = prodPrice;
    new_product->product_income = 0;
    new_product->productCopy = productCopy;
    new_product->productFree = productDestroy;
    ...
    productDestroy(new_product); //free the product in use by us because asRegister makes it's own copy of our product
...
MatamikyaResult mtmChangeProductAmount(Matamikya matamikya, const unsigned int id, const double amount)
{
    ...
    if((current_product == NULL) || (asGetAmount(matamikya->warehouse_products, current_product, &current_product_current_amount) != AS_SUCCESS))
    {
    ...
    if(!(checkIfAmountTypeIsConsistent((amount + current_product_current_amount), current_product->product_amount_type)))//check amount after addition
    {
        return MATAMIKYA_INVALID_AMOUNT;
    }
...
MatamikyaResult mtmClearProduct(Matamikya matamikya, const unsigned int id)
{
    ...
    AmountSetResult delete_item_from_warehouse_res = asDelete(matamikya->warehouse_products, product_to_delete);
    if((delete_item_from_warehouse_res == AS_ITEM_DOES_NOT_EXIST) || delete_item_from_warehouse_res == AS_NULL_ARGUMENT)
    {
        return MATAMIKYA_PRODUCT_NOT_EXIST;
    }
    return MATAMIKYA_SUCCESS;
...
unsigned int mtmCreateNewOrder(Matamikya matamikya)
{
    ...
    new_order->order_items=asCreate(productCopy, productDestroy, productCompare);
                                     ...
                                     const unsigned int productId, const double amount)
{
    ...
    if (!checkIfAmountTypeIsConsistent(amount, product_in_warehouse->product_amount_type)){
        return MATAMIKYA_INVALID_AMOUNT;
    }
    if(amount==0) {
        return MATAMIKYA_SUCCESS;
...
MatamikyaResult mtmPrintInventory(Matamikya matamikya, FILE *output)
{
    ...
    AS_FOREACH(Product, iterator, matamikya->warehouse_products){
        ...
        mtmPrintProductDetails(iterator->product_name, iterator->product_id,
                               current_amount, iterator->product_base_price, output);
    }
    return MATAMIKYA_SUCCESS;
}
...
MatamikyaResult mtmPrintOrder(Matamikya matamikya, const unsigned int orderId, FILE *output)
{
    ...
    total_price=calculateTotalPrice(current_order);
...
MatamikyaResult mtmPrintBestSelling(Matamikya matamikya, FILE *output)
{
    ...
    double temp_income = 0;
    temp_income = best_seller->product_income;
    if (temp_income == 0){
        fprintf(output, "none\n");
        return MATAMIKYA_SUCCESS;
    }
